...
- **JavaScript error on all pages**
  - Problem: `Uncaught SyntaxError: Identifier 'API_BASE_URL' has already been declared`
  - Cause: Multiple JS files declare `const API_BASE_URL`, causing conflict when loaded together
  - Solution: Added check `if (typeof API_BASE_URL === 'undefined')` before declaring
...
|------|-------------|-------------|
| `js/stats.js` | Modified | Added dynamic API_BASE_URL detection with duplicate check |
...
- Added check to prevent duplicate variable declaration
...
#### Changes Made:
1. **API Migration**: Switched from Google Gemini to GROQ API
   - More cost-effective pricing
   - Faster response times
   - Better MCQ extraction quality

2. **Model Selection by Environment**:
   - Production (`FLASK_ENV=production`): Llama 3.3 70B
   - Testing (`FLASK_ENV=testing`): Llama 3.1 8B
   - Automatic model selection based on environment

3. **Version Display**: Added version and last update time to homepage footer

#### Files Modified:
| File | Change Type | Description |
|------|-------------|-------------|
| `.env.example` | Modified | Added GROQ_API_KEY placeholder |
| `backend/config.py` | Modified | Added GROQ_API_KEY configuration |
| `backend/services/ai_processor.py` | Modified | Replaced Gemini with GROQ API |
| `backend/utils/error_handler.py` | Modified | Updated API key error suggestions |
| `frontend/index.html` | Modified | Added version and last update display |
| `frontend/css/styles.css` | Modified | Added version info styling |
| `tests/test_ai_processor.py` | Modified | Updated test expectations |
| `CHANGELOG.md` | Modified | Updated with version 2.0.0 details |

#### New Dependencies:
- `groq>=0.4.0` - GROQ API Python client

#### GROQ Models Used:
- **Production**: `llama-3.3-70b-versatile` - Best quality for production
- **Testing**: `llama-3.1-8b-instant` - Faster and cheaper for testing

#### How to Get GROQ API Key:
1. Visit https://console.groq.com/keys
2. Create a new API key
3. Add to `.env` file: `GROQ_API_KEY=your-api-key`

#### Testing Files:
- `testmcq.pdf`: Contains 11 MCQ questions
- `testmcq2.pdf`: Contains 173 MCQ questions

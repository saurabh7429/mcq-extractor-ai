...
class TestAIProcessor(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        # Model name depends on FLASK_ENV - default is production (Llama 3.3 70B)
        env = os.getenv('FLASK_ENV', 'production')
        if env == 'testing':
            self.assertEqual(self.processor.model_name, 'llama-3.1-8b-instant')
        else:
            self.assertEqual(self.processor.model_name, 'llama-3.3-70b-versatile')
